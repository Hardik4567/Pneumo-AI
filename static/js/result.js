...
(function () {
  ...
  const renderOriginalImage = (data) => {
    ...
    // Hide any placeholder sitting next to the image
    const ph = $('originalImagePlaceholder') || $('xrayPlaceholder');
  ...
  const renderHeatmap = (data) => {
    // Prefer the pre-normalised URL; fall back to raw API path
    const publicUrl =
      data.__heatmapUrl ||
    ...
    // Target: dedicated element first, legacy fallback second
    const target = $('heatmapImage') || $('originalImage');
    if (!target) {
      console.warn('[PneumoAI] No heatmap image element found (#heatmapImage / #originalImage).');
      return;
    }

    target.src = publicUrl;
    target.alt = 'Grad-CAM Heatmap';
    console.log('[PneumoAI] Heatmap rendered into #' + target.id + ' ✓ src:', publicUrl);

    const ph = $('heatmapPlaceholder') || $('xrayPlaceholder');
    if (ph) ph.style.display = 'none';
  };
